...
inline void PulseSetOutlineConstants(IShaderDynamicAPI *pShaderAPI, int vertexConstant,
	float outlineWidth, float outlineAngle)
{
	VMatrix projMatrix;
	pShaderAPI->GetMatrix(MATERIAL_PROJECTION, projMatrix.Base());
	float projScale = projMatrix[0][0];
	float clampedAngle = MIN(MAX(outlineAngle, 0.0f), 90.0f);
	float outlineParams[4] = { outlineWidth,
		fabsf(projScale) > 1e-6f ? 1.0f / projScale : 1.0f,
		cosf(clampedAngle * (3.14159265f / 180.0f)), clampedAngle > 0.0f ? 1.0f : 0.0f };
