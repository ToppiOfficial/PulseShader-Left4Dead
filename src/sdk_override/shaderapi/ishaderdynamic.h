...
public:
	...
	// PULSE OVERRIDE: L4D2's IShaderDynamicAPI has no SetVectorRenderingParameter.
	// Verified by disassembling shaderapidx9.dll: slot 27 is GetFloatRenderingParameter
	// (fld [ecx+idx*4+0x312C]), 28 GetIntRenderingParameter, 29 GetVectorRenderingParameter
	// (3 floats at +0x317C), 30 GetFlashlightStateEx, 31 GetDX9LightState, 32 GetPixelFogCombo -
	// each one slot lower than this header declares. Keeping the method here made
	// GetIntRenderingParameter land on GetVectorRenderingParameter, which took our enum
	// as its output pointer and faulted writing to address 0.
	...
	// PULSE OVERRIDE: L4D2 has no ShouldWriteDepthToDestAlpha. Verified by
	// disassembly: slot 40 is GetMatrix(mode, float*) (2 args, ret 8), 41
	// PushDeformation, 42 PopDeformation - i.e. two slots below this header,
	// where everything above is only one below. Declaring it shifted GetMatrix
	// and the deformation calls, and calling it (0 args) read a stack value as
	// an array index inside GetMatrix.
